			...
			<div class="row">
				<div class="col-xs-2">
					<label for="memberId">会員ID</label> <input type="text"
						class="form-control" id="memberId" value="${memberId}" disabled
						required> <input type="hidden" name="memberId"
						value="${memberId}">
				</div>
				<div class="col-xs-4">
					<label for="familyName">苗字</label> <input type="text"
						class="form-control" id="familyName" name="familyName"
						value="${familyName}" autocomplete="off" maxlength='10' required>
				</div>
				<div class="col-xs-4">
			...
			<div class="row">
				<div class="col-xs-2">
					<label for="postal">郵便番号</label> <input type="text"
						...
						placeholder="0000000" autocomplete="off" pattern="[0-9]{7}"
						maxlength="7" required>
				</div>
				<div class="col-xs-8">
					<label for="address">住所</label> <input type="text"
						class="form-control" id="address" name="address"
						value="${address}" autocomplete="off" maxlength="100" required>
				</div>
			</div>
			<div class="row">
				<div class="col-xs-2">
					<label for="tel">電話番号</label> <input type="text"
						...
						pattern="\d{2,4}-\d{3,4}-\d{3,4}" maxlength="20" required>
				</div>
				<div class="col-xs-4">
					<label for="email">メールアドレス</label> <input type="text"
						class="form-control" id="email" name="email" value="${email}"
						placeholder="my@address.com" autocomplete="off" maxlength="100"
						...
						<div class="col-xs-4">
							<select class="form-control" id="year" name="year">
								<c:forEach begin="1900" end="2030" var="i">
									<option <c:if test="${i == year}">selected</c:if>
										value="<fmt:formatNumber value="${i}" pattern="0000" />">
						...
						<div class="col-xs-4">
							<select class="form-control" id="month" name="month">
								<c:forEach begin="0" end="11" var="i">
									<option <c:if test="${(i + 1) == month }">selected</c:if>
										value="<fmt:formatNumber value="${i + 1}" pattern="00" />">
							...
							<select class="form-control" id="date" name="date">
								<c:forEach begin="1" end="31" var="i">
									<option <c:if test="${i == date }">selected</c:if>
										value="<fmt:formatNumber value="${i}" pattern="00" />">
										<c:out value="${i}" /></option>
								</c:forEach>
							</select>
						</div>
